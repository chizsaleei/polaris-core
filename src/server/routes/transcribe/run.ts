...
const DEFAULT_EXPIRY = 900; // client hint only
  ...
  (req: UploadRequest, res: Response) => {
    void runWithRequestContext({ headers: req.headers, user_id: req.user?.userId }, async () => {
      try {
        ...
        // Supabase v2 createSignedUploadUrl signature accepts (path) only
        ...
        return res.status(200).json({
          ...
          data: {
            ...
            // client hint for when to use the URL; Supabase controls real expiry
